...
void LoadWater()
{
	VirtualProtect((void*)0x5C1710, sizeof(BYTE), PAGE_READWRITE, NULL);
	VirtualProtect((void*)0x69CC58, sizeof(float), PAGE_READWRITE, NULL);
	VirtualProtect((void*)0x69CD7C, sizeof(float), PAGE_READWRITE, NULL);
	VirtualProtect((void*)0x69CD80, sizeof(float), PAGE_READWRITE, NULL);
	VirtualProtect((void*)0x69CDCC, sizeof(float), PAGE_READWRITE, NULL);

	BYTE * WaterRender = (BYTE *)0x5C1710;							 // Function of water rendering
	if (*WaterRender == 0xDE) WaterRender += 0x20;					 // VC 1.1
	BYTE   WaterLines = 40;											 // count of lines of external water 5
	     //   *(float *)0x69CC58 = 0;		    					 // move all water to the west (400). 
	*(float *)0x69CD7C = 10000;										 // (z-15)*800*0.022+1200  the radius of visability of internal water lower 60 meters 
	*(float *)0x69CD80 = 10000;										 // the radius of visability of internal water higher 60 meters 
	*(float *)0x69CDCC = WaterLines * 256;						     // the move on count of external water (1280 = 5*256)
	ProtectBYTE(WaterRender + 0xDC1, WaterLines);			         // count of external water from the north and south
	ProtectBYTE(WaterRender + 0xDCB, (WaterLines - 5) * 2 + 26);     // count of external water from the north and east
	ProtectBYTE(WaterRender + 0xDD3, WaterLines);					 // count of external water from the west and east on north
	ProtectBYTE(WaterRender + 0x11D1, WaterLines);					 // count of external water from the west and east
	ProtectBYTE(WaterRender + 0x11DB, WaterLines + 16);				 // increasing of external water of west and east to north
} 
